...
class QuantityPredictionModel:
    def __init__(self):
        ...
        self.model_path = os.path.join(base_dir, 'models', 'quantity_prediction_model.joblib')
        self.scaler_path = os.path.join(base_dir, 'models', 'quantity_scaler.joblib')
        self.label_encoders_path = os.path.join(base_dir, 'models', 'quantity_label_encoders.joblib')
        self.feature_columns_path = os.path.join(base_dir, 'models', 'quantity_feature_columns.joblib')
        
    def load_and_preprocess_data(self):
        """Load and preprocess the data for quantity prediction"""
        try:        
            base_dir = os.path.dirname(os.path.abspath(__file__))
            ...
            # Price-based features that might influence quantity
            df['price_efficiency'] = df['Unit Selling Price (RMB/kg)'] / df['Wholesale Price (RMB/kg)'].fillna(1)
            df['adjusted_price'] = df['Unit Selling Price (RMB/kg)'] * (1 + df['Loss Rate (%)'].fillna(0) / 100)
            
            # Define feature columns for quantity prediction
            self.feature_columns = [
                'Unit Selling Price (RMB/kg)', 'Wholesale Price (RMB/kg)', 
                'Loss Rate (%)', 'month', 'dayofweek', 'hour', 'is_weekend',
                'price_efficiency', 'adjusted_price',
    ...
    def train(self):
        """Train the quantity prediction model"""
        try:
            ...
            # Train model with parameters optimized for quantity prediction
            ...
            print(f"Quantity Model R² score on training data: {train_score:.4f}")
            print(f"Quantity Model R² score on test data: {test_score:.4f}")
    ...
    def predict(self, features):    
        """Make quantity predictions for new data"""
        try:
            ...
            for col in self.feature_columns:
                if col in features.columns:
                    ...
                    if col in ['month', 'dayofweek', 'hour', 'is_weekend']:
                        prediction_df[col] = 0  
                    elif col in ['price_efficiency', 'adjusted_price']:
                        prediction_df[col] = 1.0  # Default multipliers
                    else:
                        prediction_df[col] = 0  
            ...
            for col in self.label_encoders:
                if col in prediction_df.columns:
                    try:
                        ...
                        # Use first class if unknown category
